...
set < int > Artic;
int IdBCC[MAXN], sizeBCC[MAXN], DfsCounter, NumBCC;
struct
{
...
void extract(int node, int now)
{
    ...
    while(S.top() != pp)
    {
        temp = S.top();
        S.pop();
        if (IdBCC[temp.first] != NumBCC) ++count;
        if (IdBCC[temp.second] != NumBCC) ++count;
        IdBCC[temp.first] = IdBCC[temp.second] = NumBCC;
        BCC[NumBCC].push_back(temp);
    }
    sizeBCC[NumBCC] = count;
...
void printRes()
{
    ...
    printf("Number of BCCs : %d\n", NumBCC);
    for (i=1; i<=NumBCC; ++i)
    {
        printf("%d - %d : ", i, sizeBCC[i]);
        for (j=0; j<BCC[i].size(); ++j) printf("(%d %d) ", BCC[i][j].first, BCC[i][j].second);
        puts("");
    }
    puts("");
}
